...
export default function App() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/applications/${appToDelete.id}`, { method: 'DELETE' });
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
      ...
      if (selectedDetailId === appToDelete.id) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex" id="app-viewport-root">
      ...
      <div className="flex-1 flex flex-col" id="main-content-layout">
            ...
            <div className="max-w-6xl mx-auto px-8 pt-6" id="server-connection-error">
              <div className="p-5 bg-rose-50 border border-rose-200 rounded-2xl text-rose-700 flex items-start gap-4 shadow-sm text-left">
                ...
                <div className="space-y-1">
                  <h4 className="font-extrabold text-sm text-slate-900 tracking-tight">System Connection Alert</h4>
                  <p className="text-sm text-slate-600 leading-relaxed">
                    {errorMessage} Please check your network connection or verify that the repository services are currently reachable.
                  </p>
                  <button
            ...
            <div className="max-w-6xl mx-auto px-8 py-6 space-y-6" id="applications-grid-container">
              {loading && applications.length === 0 ? (
                ...
                <div className="py-20 border border-slate-200/60 border-dashed rounded-3xl text-center space-y-5 bg-white shadow-xs max-w-xl mx-auto my-12" id="grid-empty-state">
                   ...
                   <div>
                     <h3 className="font-sans font-bold text-lg text-slate-900">No applications registered.</h3>
                     <p className="text-slate-500 text-sm mt-1.5 max-w-md mx-auto leading-relaxed">
                       Store, catalog, and manage integrated enterprise application knowledge here to design GenAI metrics.
                     </p>
                   </div>
                   <button
                     onClick={() => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="px-6 py-6 border-b border-slate-100 space-y-2">
              <h3 className="font-sans font-bold text-lg text-slate-900">Delete Application</h3>
              <p className="text-sm text-slate-500">
                Are you sure you want to delete <span className="font-semibold text-slate-800">{appToDelete.name}</span>? This action cannot be undone.
              </p>
            </div>
            <div className="px-6 py-4 bg-slate-50 flex items-center justify-end space-x-3">
              ...
              <button
                ...
                className="px-4 py-2 text-sm font-semibold text-white bg-rose-600 hover:bg-rose-700 rounded-xl shadow-xs cursor-pointer transition-colors"
              >
                Delete
              </button>
            </div>
